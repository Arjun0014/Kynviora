...
## DEC-137 - The app follows the system's appearance, because `userInterfaceStyle` was still `light`

**Context.** DEC-130 added a dark theme at equal rank with the light one and made `FOLLOW_SYSTEM`
the default appearance choice. `apps/mobile/app.json` still carried `"userInterfaceStyle": "light"`,
written under DEC-101 when there was no `DARK_THEME` and thirty-four files imported `LIGHT_THEME`
directly.

That is not a stale comment. `expo-system-ui`'s config plugin writes the value into
`res/values/strings.xml` as `expo_system_ui_user_interface_style`, and
`SystemUIReactActivityLifecycleListener.onCreate` reads it and calls
`AppCompatDelegate.setDefaultNightMode(MODE_NIGHT_NO)` on every activity create. React Native's
`useColorScheme()` reads the activity's `Configuration.uiMode`, which that has just forced. So
`useColorScheme()` returned `'light'` on a phone in dark mode, `resolveThemeName('FOLLOW_SYSTEM',
'light')` resolved to `LIGHT`, and **the dark theme was unreachable from the default state**. Only
somebody who went to You then Accessibility and chose `DARK` explicitly ever saw it.

**Options.** (a) Leave it and document that dark is opt-in. (b) `"automatic"`, which the plugin maps
to `MODE_NIGHT_FOLLOW_SYSTEM`. (c) Remove the key, which the plugin treats as absent and also maps
to follow-system, less explicitly.

**Decision.** `"userInterfaceStyle": "automatic"`.

**Rationale.** (a) is a design document describing a product that does not exist: DEC-130's whole
claim is that both themes are first class and the default is the system's, and one line of
configuration was making the second half false. (c) works and says less; a project whose appearance
policy is a written decision should state it in the configuration rather than rely on a default.

**Consequences.** `strings.xml` is regenerated by `expo prebuild`, so the committed change is
`app.json` alone - `apps/mobile/android/` is prebuild output and is gitignored. A device carrying an
older build keeps the old string until it is rebuilt, which is why this is verified by looking at a
dark-mode emulator rather than by reading the file. The three appearance choices are unchanged;
what changes is which theme `FOLLOW_SYSTEM` can resolve to.

**Sources.** `expo-system-ui/plugin/build/withAndroidUserInterfaceStyle.js`,
`expo-system-ui/android/src/main/java/expo/modules/systemui/singletons/SystemUI.kt`, DEC-101,
DEC-130, `DEV-077`.

---

## DEC-138 - The coverage statement is the first thing on the Safety screen, not the last

**Context.** `09` requires a coverage statement to accompany every result rather than be inferred
from its absence, and `23` D-014 forbids an absence of a matched rule rendering as approval. The
Safety screen satisfied both by drawing the sentence underneath the list.

**Options.** (a) Leave it under the list. (b) Put it above the list. (c) Both.

**Decision.** Above the list, once, as a card in `informational` at the same visual rank as the
results.

**Rationale.** Two things are wrong with underneath. On a shelf of five identical "not enough
information" lines the conclusion is formed **while scanning**, so a qualification arriving
afterwards arrives after the thing it was meant to qualify. And on a longer shelf it is below the
fold - and for a sentence whose entire job is to stop a misreading, below the fold is the same as
absent. The design system's own rule is that a limitation never moves down a layer; below the fold
is a layer.

(c) was rejected because a screen reader would announce it twice and a person would read the second
one as a different statement.

**Consequences.** `verify:device:safety` `SAF-3` and `SAF-6` are unaffected - `collectScreenText`
scrolls the whole screen, so neither was ever measuring position. What they measure is that the
sentence is present in the unfiltered state and survives a filter that matches nothing, and both
still hold, more strongly: the card is drawn unconditionally rather than after a list that may be
empty.

**Sources.** `09`, `18`, `23` D-014, `docs/design/DESIGN_SYSTEM.md` section 10,
`scripts/device/safetyInbox.ts`.

---

## DEC-139 - The haptic engine is React Native's `Vibration`, on Android only

**Context.** `DEV-070` recorded haptics as designed, wired and not felt, blocked on a manifest
change: React Native's `Vibration` needs `android.permission.VIBRATE`, a newly declared permission
appears on the Play listing and in the Data-safety form, and that is a product decision rather than
something to take as a side effect of a styling pass.

**The premise was false.** `VIBRATE` is already declared. The manifest merger report attributes it
to the app's own `AndroidManifest.xml`, where `expo-notifications`' config plugin puts it, and
`dumpsys package com.kynviora.app` lists it among the requested permissions of the build installed
on the emulator today. Nothing had to be added.

**Options.** (a) Stay blocked. (b) `expo-haptics`, which is a native module needing a prebuild and a
new dependency. (c) React Native's own `Vibration`, which needs neither.

**Decision.** (c), behind the existing adapter, and **only on Android**.
`createVibrationHapticEngine` answers `null` on every other platform and the recording engine stays
installed there.

**Rationale.** (b) is the better API on iOS and buys nothing on Android that the four short patterns
here need. (c) ships today with no new dependency, no new permission and no prebuild - and the
adapter means swapping it later is one file.

iOS gets nothing rather than something because `Vibration.vibrate()` on iOS takes no duration: every
call is the same full-length buzz, so a `selection` tick - by far the most frequent intent - would
be that buzz on every press of every chip. iOS's real answer is `UIImpactFeedbackGenerator`, which
this build does not have. An absent haptic costs a little polish and no information (DEC-131); a
wrong one is a product somebody turns off.

The patterns escalate by **pulse count** rather than by length: one for something that happened, two
for something to look at, three for something that did not work. Length alone is not distinguishable
through a pocket or by somebody with reduced sensation. `selection` and `confirm` share a count on
purpose and are not meant to be told apart by feel - both accompany something the person just did
and can see.

**Consequences.** `DEV-070` closes. Nothing changes about DEC-131: a haptic still never reports
anything on its own, and there is still no `error` intent for a call site to reach for. The merged
manifest must be re-checked after this, which `verify:device:camera` `CAM-3` already does for the
eight permissions it counts - `VIBRATE` is not one of them, because it is a normal permission with
no runtime prompt.

**Sources.** `12`, `18`, DEC-131, `DEV-070`, `apps/mobile/src/platform/vibrationHaptics.ts`.

---

## DEC-140 - A dose recorded by voice with no signal goes into the app's own journal, and the confirmation is for the record rather than for the transport

**Context.** `DEV-071`: `record_dose` is described in the registry as `offline: QUEUES` and the voice
path did not queue it. The recorded blocker was not the wiring - which is small - but a semantic
question: a queued dose is a promise about a later moment, and the confirmation a person gave was
for "record this", not for "keep this here and send it when you can". Making the change without
saying it in the summary would be confirming one sentence and performing another, which is the
failure DEC-134 exists to prevent.

**Options.** (a) Leave it unqueued and keep saying the honest sentence. (b) Put the possibility into
the confirmation summary: "record that you took it, and keep it here if there is no connection".
(c) Ask a second question after the send fails. (d) Queue it, and say so in the **report**.

**Decision.** (d). **The summary describes the intent; the report describes what happened.**

**Rationale.** A confirmation is for the record, not for the transport. When somebody says "record
that I took it", what they are agreeing to is the record being made; whether it travels now or in
ten minutes is a fact about the phone's radio. Queueing performs exactly the sentence that was
confirmed, and DEC-134's rule - that what was agreed to is the sentence that was heard - is kept
rather than bent, because nothing about the intent changed.

(b) puts a conditional about the radio into every confirmation, including the overwhelming majority
where there is a connection, and the confirmation sentence is the one that must be understood.
(c) is the worst of the four for the audience Voice Mode exists for: it makes somebody answer a
question about network transport at the moment they are least able to, and the safe reading of an
unanswered question is that the dose is lost. (a) is what shipped, and it lost doses.

`UTTERANCES.queued` already existed and is exactly this sentence. It is spoken **instead of**
"Done.", never as well - the same distinction the dose sheet already draws between "Recorded." and
its offline note, which `OFF-6` measures on a device.

**Three answers, not two.** The old code queued nothing and said "this phone has no connection" for
_every_ failure. So a caregiver whose grant had been revoked was told to try again later, and a
retry would be refused identically. Only `OFFLINE` means no server was reached and only `OFFLINE`
queues; everything else is a server that answered and declined, and is reported as a refusal.

**The queue is the one that already exists.** `usePendingSync().queue` is the identical call the
dose sheet makes, under the identical entity type and the identical key - so `13`'s per-entity
policy, the retry budget, the drain and `PendingSenders`' wire call are all the ones `OFF-6` and
`OFF-7` already exercise on hardware. Voice adds no sync mechanism; it reaches the one that exists.
That is DEC-132 applied to the offline path.

**A fourth channel, so a fixed sentence cannot cite itself.** `ToolResult` gains `utterances`, a
list of keys into the Speech Gate's closed set. `spoken` is checked against `composedFrom`, and the
caller passes the executor's own `spoken` array as `composedFrom` - so a string written in the
executor cited itself and passed. Every one of them happened to come from the presentation layer,
and nothing was enforcing it. A key cannot cite itself: `gateSpeech` resolves it against
`UTTERANCES` and refuses one this build does not have.

**Consequences.** `DEV-071` closes. A `false` from the journal is a real answer - the store may not
be open, or the device may have no room (`19`'s low-storage scenario, `DEV-055`) - and it produces
the honest "no connection" sentence rather than the queued one, because a person told their dose was
kept when nothing kept it stops thinking about a record that does not exist.

**Sources.** `12`, `03` group J, DEC-111, DEC-132, DEC-134, DEC-135, `DEV-071`, `DEV-055`.

---

## DEC-141 - The server reports what a caller may do on a profile, because voice offers everything from one place

**Context.** `DEV-074`: a caregiver granted `MANAGE_MEDICINES` could set a reminder time by touch and
was told to use the screen by voice. `capabilitiesFor` gave an owner everything and gave a caregiver
the read capabilities plus whatever a screen had happened to report - and `VoiceProvider` passed
neither of the two answers that existed, so a caregiver got the three reads and nothing else,
always.

**Options.** (a) Assume a capability and let the route refuse. (b) Thread `mayRecordDoses` from the
shelf and `mayEdit` from an item detail into the dispatch context as they arrive. (c) A route that
reports the caller's capabilities for a profile.

**Decision.** (c). `GET /v1/profiles/:profileId/capabilities`, answered with
`kynviora.has_capability` over `CAREGIVER_CAPABILITIES` in one query, returning only what is held.

**Rationale.** (a) is a person completing a spoken form for nothing, which is the failure DEC-116's
screen half exists to prevent, arriving through a different door. (b) is what `DEV-074` proposed as
the smaller fix and it is smaller, but it makes what voice may offer depend on which tabs somebody
happened to open - which is the same defect `PendingSenders` was moved off the screens to avoid
(`DEV-044`), and it still leaves `MANAGE_PERSONAL_CARE` answerable only by opening a personal-care
item. A surface that offers everything from one place needs the whole set at once.

**Why it discloses nothing.** Every answer is about the caller's own authorization and
`has_capability` consults `kynviora.current_user_id()` - there is no parameter that could name
somebody else. A profile the caller cannot see answers with an empty list, which is what an empty
shelf page already tells them, so an unknown ID and a profile with no grant are indistinguishable.
`13`'s rule that the API does not say "you are not allowed" is about a **refusal** carrying a reason;
this is the affirmative report the same chapter requires so a screen can withhold a control rather
than offer one the write would refuse. `mayRecordDoses` on the shelf page has been exactly this
since DEC-116, and this is that decision generalised.

**Only what is held.** A list of every capability with a boolean beside it is the same information
in a shape that invites a client to render "you cannot do this" - the sentence `14` and DEC-045 both
keep off a screen.

**The two vocabularies stay two.** `caregiver_grant.capabilities` is what an owner can grant;
`ToolCapability` is what a tool can require. `capabilities.ts` maps between them with a lookup that
has **no default**, so a grant capability with no tool behind it - `VIEW_CARE`, `EXPORT_SUMMARY`,
`MANAGE_CAREGIVERS` - contributes nothing rather than being mapped to something adjacent, and a
capability a newer server invents is ignored rather than throwing.

**Consequences.** `DEV-074` closes. This authorises nothing: every route checks again on the session
against row-level security. The read is not made for an owner (`capabilitiesFor` short-circuits on
ownership, so the round trip would change nothing) and not made with Voice Mode closed, so it is off
every cold start. What is still narrower by voice is in the registry rather than here, and each has
a voice-specific reason recorded in `docs/design/VOICE_MODE.md` section 3.

**Sources.** `11`, `13`, `14`, DEC-045, DEC-116, DEC-132, `DEV-074`, `DEV-044`.

---

## DEC-142 - Care uses `Screen` like every other destination

**Context.** Care was the one destination with a hand-rolled frame: its own `SafeAreaView`, its own
`ScrollView`, its own heading block and its own padding - with a **second** `ScrollView` nested
inside the first, in `CaregiverAccessList`. The stated reason was that `Screen` draws a destination
heading and Care had never had one.

That reason expired on 2026-09-07. Removing the navigator's header (`DEV-076`) gave Care a heading
of its own, because `18` requires one.

**Decision.** Care renders through `Screen`, in all three of its branches, and `CaregiverAccessList`
renders content rather than a page.

**Rationale.** Two scroll views nested in the same direction is one of them eating the other's
gestures, which is the shape of `DEV-046` - a control below the fold with no way to reach it - and
is not something any test in this repository can see. Beyond that, five destinations with four
frames is four places for a safe-area inset, a pull-to-refresh and a heading role to drift; `Screen`
exists so that is one decision.

**Consequences.** Care gains the profile eyebrow (`06` requires the current person to be clear on
every screen showing care data, and Care is entirely care data), pull-to-refresh, and the `canvas`
ground that makes a card read as a card - it was painting `surface`, so every card was the same
colour as the page behind it. `verify:device:a11y` surveys the invitation sheet at both font scales
and is the check that keeps `DEV-046` fixed.

**Sources.** `06`, `18`, `DEV-046`, `DEV-076`, DEC-130.

---

## DEC-143 - You is sectioned by subject, and the health context is gated on its own read

**Context.** `06` names five subjects for the You destination - account, accessibility, privacy,
notifications, data. The screen was a flat stack of eight feature blocks with nothing saying where
one subject ended and the next began.

**Decision.** Section markers per subject, using the shared `SectionHeader`; the identity strip
becomes a card; and the health context moves out from under the notification-settings gate.

**Rationale.** A flat stack reads as an admin form, and this is the screen holding consent, what
leaves the device, and how to close an account. The markers are also headings a screen reader
navigates by, which is the half of "sectioned" that is not decoration.

The order inside the sections is unchanged wherever a comment already gave a reason, and those
reasons stay beside the blocks they belong to: appearance above everything needing a network answer,
the person above every setting about them, consent above the notification dials whose meaning it
decides, and the two irreversible things last. `PendingQueue` gets **no** marker on purpose: a
heading that appeared and disappeared with the journal would be a landmark a screen reader could not
navigate to twice, and the block names itself.

**The health context move is a defect fix, not a layout preference.** It sat inside
`activeProfileId === null || settings === null || policy === null ? <ResourceState/> : <>...</>` -
so a household whose **notification settings** would not load lost a health context that had arrived
perfectly well. That is a partial state rendered as an absence, which is the exact failure `06`
names that state to prevent. It is gated on its own resource now, and it sits with the person it is
about rather than between two notification blocks - which is what its own comment always said and
what its placement did not.

**Consequences.** No copy changes and no control moves between screens. `verify:device:a11y` surveys
